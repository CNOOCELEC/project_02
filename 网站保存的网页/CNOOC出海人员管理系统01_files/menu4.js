...
Menu.prototype.cssFile = "skin/custom.css";
...
MenuItem.prototype.dispatchAction = function () {
	...
	if ( this.subMenu ) {
	  ...
	  for(i=0; i<this.subMenu.items.length; i++){
	    //alert(this.subMenu.items[i].action);
	  }
	  
	  //alert('dispatch');
	  
	  if(parent.workareaFrame){
	    if(parent.workareaFrame.main_toolbar){
	      //parent.workareaFrame.main_toolbar.clearSubMenu(this.subMenu.items[0].text);
	      parent.workareaFrame.main_toolbar.disposeTab();
	      
	      for(i=0; i<this.subMenu.items.length; i++){
	        parent.workareaFrame.main_toolbar.addSubMenu(this.subMenu.items[i].text,this.subMenu.items[i].action,this.subMenu.items[i].icon);
	      }
	      parent.workareaFrame.main_toolbar.showTab();
	    }
	  }
...
MenuButton.prototype.toHtml = function () {
	...
	return	"<span id=\""+this.oSubMenu.id+"\" unselectable=\"on\" " +
			(cssClass != "" ? " class=\"" + cssClass + "\"" : "") +
			(toolTip != "" ? " title=\"" + toolTip + "\"" : "") +
			(!this.visible ? " style=\"display: none\"" : "") +
